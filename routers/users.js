...
async function post(req, res) {
  ...
  if (validation.error) {
    return res.status(400).send(getValidationMessage(validation));
  }
  ...
  return res
    .header("x-auth-token", userInsert.generateAuthToken())
...
module.exports = router.get("/me", authMiddleware, get).post("/", post);
